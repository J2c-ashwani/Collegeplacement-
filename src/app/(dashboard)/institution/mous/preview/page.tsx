    ...
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge className="bg-blue-50 text-blue-800 border-blue-200 text-xs font-mono">
            STAGE 4: MOU — READY FOR SIGNATURE ({rendered.mouReference})
          </Badge>
          <Badge className="bg-emerald-50 text-emerald-800 border-emerald-200 text-xs font-mono">
            STAGE 7: EXECUTED &amp; ACTIVE PARTNERSHIP
          </Badge>
        </div>
      </div>
      ...
      <div className="p-3 rounded-md border border-slate-200 bg-white space-y-2 print:hidden">
        <div className="flex items-center justify-between text-[11px]">
          ...
          <span className="font-mono text-[10px] text-emerald-800 font-semibold">
            Current State: Stage 7 — Active Partnership (Signed &amp; Counter-Executed)
          </span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-7 gap-1.5">
          {MOU_LIFECYCLE_STAGES.map((stage) => (
            <div
              key={stage.code}
              className="p-1.5 rounded border border-emerald-200 bg-emerald-50/60 text-emerald-950 text-[10px]"
            >
      ...
      <div className="bg-white border border-slate-300 shadow-sm rounded-md p-8 sm:p-10 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b-2 border-slate-900 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-600">
              Automatically Generated from Verified Institutional Onboarding Records &bull; Lifecycle: MoU Generated (Ready for Signature) &rarr; Signed / Executed MoU
            </p>
          </div>
        ...
        {/* Dual Digital Execution Block with Stage 4 vs Stage 6 Timestamps in IST (UTC) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t-2 border-slate-900 text-xs">
          ...
          <div className="p-4 rounded bg-slate-50 border border-slate-200 space-y-1">
            <div className="text-[10px] font-mono uppercase text-slate-500">
              SECOND PARTY — PARTNER INSTITUTION (STAGE 6: SIGNED / EXECUTED MOU)
            </div>
            ...
            <div className="font-mono text-[11px] text-emerald-700">
              Terms Accepted &amp; Counter-Executed: {formatDualTimestamp(fallbackSubmission.termsAcceptedAt)}
            </div>
          </div>
        </div>
      </div>
